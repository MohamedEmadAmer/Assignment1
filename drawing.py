...
def rectangle_will_fit(x, y, length, height):
    #"THIS FUCTION WILL CHECK FOR THE CORDINATES OF THE RECTANGLE"
...
def draw_shape(shape, color_code, x, y, length, height=0):
    #"THIS FUCTION WILL DRAW ALL OF THE THREE SHAPES"
    if shape == "r":
        # "THIS FUCTION WILL DRAW A RECTANGLE"
        if x + length < +400 and y + height <= 400:
            t.up()
            t.goto(x, y)
        ...
        # "THIS FUCTION WILL DRAW A CIRCLE"
        if height <= 400:
            t.up()
            t.goto(x, y)
            t.lt(180)
        ...
        # "THIS FUCTION WILL DRAW A TRIANGLE"
        t.up()
        t.fillcolor(color_code)
        t.goto(x, y)
        t.begin_fill()
...
draw_shape("r", "red", 100, 250, 300, 150)
